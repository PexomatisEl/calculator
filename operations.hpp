...
void praxis(std::vector<std::string>& string1, bool& greenFlag)
{
    ...
    for (int i = 0; i < countPower; i++)
    {
        ...
        double base = std::stod(string1[indexFirstPower - 1]);
        double power = std::stod(string1[indexFirstPower + 1]);
        double doubleResult(0);
        doubleResult = pow(base, power);

        if (greenFlag && int(power)%2 == 0)
        {
            doubleResult *= (-1);
            greenFlag = false;
        }
        ...
        string1.emplace(iteratorFirstPower - 1, strResult);
    ...
    for (int i = 0; i < countMultDiv; i++)
    {
        ...
        std::string strResult = std::to_string(doubleResult);
        string1.emplace(iteratorFirstMultDiv - 1, strResult);
    ...
    for (int i = 0; i < countAddSub; i++)
    {
        ...
        std::string strResult = std::to_string(doubleResult);
        string1.emplace(iteratorFirstAddSub - 1, strResult);
